const logoutButton = new LogoutButton(); //? Что значат 3 точки под  LogoutButton() (vs code)
...
const ratesBoard = new RatesBoard(); //? Почему RatesBoard() коричневый, а LogoutButton() синий? (vs code)
...
moneyManager.addMoneyCallback = (data) => {
  ApiConnector.addMoney(data, (response) => {
    if (response.success) {
      ProfileWidget.showProfile(response.data);
      moneyManager.setMessage(true, "Поднял бабло");
    } else {
